...
echo "=== 3-Node APEX RL Training Launcher (SFT-seen, TIS truncate, no RS) ==="
echo "  SFT-seen law tasks only (70), TIS truncate mode, no RS veto"
...
export APEX_TRAIN_DATA="${APEX_TRAIN_DATA:-/data/apex_docker_tasks_law_sft_seen.jsonl}"
APEX_ROLLOUT_BATCH_SIZE="${APEX_ROLLOUT_BATCH_SIZE:-8}"
...
SAVE_DIR="/data/Qwen3-30B-A3B-sft-opus-mix-lr5e5-iter113_rl_3node_sft_seen"
mkdir -p "${SAVE_DIR}"
...
sudo docker run -d --name slime-head \
    ...
    "${ENV_COMMON[@]}" \
    "${SLIME_IMAGE}" \
    bash /data/run-3node-sft-seen.sh
