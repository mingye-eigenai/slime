...
GRADING_DIR = "/data/mingye_b200-1/archipelago/grading"
GRADING_VENV = "/data/grading_venv"
TASKS_AND_RUBRICS = "/data/apex-agents/tasks_and_rubrics.json"
...
def get_or_create_world_snapshot(world_id, eval_dir, cache_dir):
    ...
    cache_path = os.path.abspath(os.path.join(cache_dir, f"{world_id}.zip"))
...
def main():
    ...
    if args.judge_model.startswith("anthropic/"):
        # For Anthropic models, use the Anthropic SDK directly via litellm.
        # Set ANTHROPIC_API_KEY env var; do NOT pass api_base (which would
        # route through an OpenAI-compatible proxy that may not support Anthropic).
        anthropic_key = args.judge_api_key or os.environ.get("ANTHROPIC_API_KEY", "")
        if anthropic_key:
            os.environ["ANTHROPIC_API_KEY"] = anthropic_key
    elif args.judge_model.startswith("openai/"):
        # For OpenAI models (gpt-5-nano, gpt-5-mini, etc.), use the OpenAI SDK
        # directly via litellm. Set OPENAI_API_KEY env var.
        openai_key = args.judge_api_key or os.environ.get("OPENAI_API_KEY", "") or DEFAULT_OPENAI_API_KEY
        if openai_key:
    ...
    # Pass reasoning_effort for models that support it (e.g. gpt-oss-120b)
    if args.judge_model.startswith("eigenai/"):
        judge_extra_args["reasoning_effort"] = "high"
