#!/usr/bin/env python3
"""
Convert apex_sft_data_grouped gemini-3.1-pro-preview trajectories to per-domain JSONL.

Same best-trajectory-per-task logic as conversion_domain_split.py:
  - For each task: pick rollout with min error_ratio, tie-break by fewest steps
  - Domain from task prompt -> mercor_training task.json -> world name prefix

Input structure: task_*/gemini-3.1-pro-preview/rollout_*.json

Usage:
    python conversion_gemini_domain_split.py \
        --input-dir /data/apex_sft_data_grouped \
...
def build_prompt_to_domain(tasks_dir: str, worlds_dir: str) -> dict[str, str]:
    ...
    logger.info("Built domain lookup: %d prompts, domains: %s",
...
def main():
    ...
    parser.add_argument("--prefix", default="apex_sft_gemini")
    ...
    stats = {"matched": 0, "unmatched": 0, "no_gemini": 0, "convert_fail": 0}

    for task_dir in task_dirs:
        gemini_path = os.path.join(args.input_dir, task_dir, MODEL_NAME)
        if not os.path.isdir(gemini_path):
            stats["no_gemini"] += 1
            continue

        rollout_files = sorted(glob.glob(os.path.join(gemini_path, "rollout_*.json")))
        if not rollout_files:
            stats["no_gemini"] += 1
            continue

        # Get user_task for domain lookup
        user_task = None
        ...
        # Select best rollout: min error_ratio, tie-break fewest steps
        best_file = None
    ...
    mix_path = os.path.join(args.output_dir, f"{args.prefix}_mix.jsonl")
    with open(mix_path, "w") as f:
        for row in all_rows:
            f.write(json.dumps(row, ensure_ascii=False) + "\n")
    logger.info("Wrote %d samples -> %s", len(all_rows), mix_path)
