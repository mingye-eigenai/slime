...
Combined from conversion_skip_missing_result.py and conversion_v3.py.

Input:  Directory with nested structure: task_*/model_name/rollout_*.json
Output: JSONL with 'messages' column in Qwen3 structured format

Key features:
  - Tool definitions loaded from tool_schemas.json, embedded in system prompt
    as Qwen3 native <tools>...</tools> XML block
  - Thinking/reasoning  -> assistant message 'reasoning_content' field
  - Tool calls          -> assistant message 'tool_calls' list field (with 'id')
  - Tool results        -> role: "tool" messages with 'content' and 'id'
  - Uses single 'id' key for both tool_calls and tool results (no 'tool_call_id')
  - Skips trajectories where a step has mismatched tool_call / tool_result counts
  - Adds step_loss_mask: 0 to assistant+tool messages in steps with errors

Usage:
    python conversion_combined.py \\
        --input-dir /data/apex_sft_least_turns \\
        --tool-schemas /data/tool_schemas.json \\
        --output /data/apex_sft_least_turns.jsonl
"""
...
def build_tool_definitions(tool_names: list[str]) -> list[dict]:
    """Build tool definitions, looking up full schemas when available."""
    defs = []
...
def format_tools_for_system_prompt(tools: list[dict]) -> str:
    """Format tool definitions as Qwen3-style <tools> block for the system prompt."""
    if not tools:
        return ""
...
def is_tool_result_error(text: str) -> bool:
    ...
    Two error patterns:
    1. Text starts with 'Internal error:' (MCP tool invocation error)
    2. code_exec JSON with "success": false
    """
...
def step_to_messages(step_entries: list[dict]) -> list[dict]:
    ...
    Uses 'id' consistently on both tool_calls and tool result messages.
    Detects errors in tool results and adds step_loss_mask: 0 to all
    messages in the step if any tool result is an error.
    """
    messages: list[dict] = []
    ...
    tool_results: list[tuple[str, str]] = []  # (ref, text)
    final_answer_text = ""
    ...
    # Detect if any tool result in this step is an error
    ...
    # Tool results -> individual role:"tool" messages with 'id'
...
def convert_trajectory(entries: list[dict]) -> list[dict] | None:
    ...
    Skips trajectories where any step has mismatched tool_call / tool_result counts.
    """
...
def main():
    ...
    parser.add_argument(
        "--input-dir",
        required=True,
        help="Directory with nested structure: task_*/model_name/rollout_*.json",
    )
    parser.add_argument(
        "--output",
        required=True,
    ...
    logger.info(
        "Total messages with step_loss_mask=0: %d", masked_steps_total
    )
