...
MODEL_CONFIGS = {
    ...
    "qwen3_30b_combined_rewritten_lr2e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8000/v1/chat/completions",
        "model": "/data/Qwen3-30B-A3B-sft-augmented-combined-v0-rewritten-lr2e5-hf",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    ...
    "qwen3_30b_rl_iter149": {
        "api_type": "openai",
        "api_url": "http://localhost:8008/v1/chat/completions",
        "model": "/data/home/mingye/Qwen3-30B-A3B-sft-opus-mix-lr5e5-iter113-rl-iter149",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    "qwen3_30b_rl_iter149_p2": {
        "api_type": "openai",
        "api_url": "http://localhost:8009/v1/chat/completions",
        "model": "/data/home/mingye/Qwen3-30B-A3B-sft-opus-mix-lr5e5-iter113-rl-iter149",
        "api_key": "dummy",
    ...
    "qwen3_30b_zhanlin_v2_e3": {
        "api_type": "openai",
        "api_url": "http://localhost:8008/v1/chat/completions",
        "model": "/data/home/mingye/Qwen3-30B-A3B-sft-zhanlin-v2-e3",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    "qwen3_30b_zhanlin_v2_e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8008/v1/chat/completions",
        "model": "/data/home/mingye/Qwen3-30B-A3B-sft-zhanlin-v2-e5",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    "qwen3_30b_zhanlin_v2_lr2e5_e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8008/v1/chat/completions",
        "model": "/data/home/mingye/Qwen3-30B-A3B-sft-zhanlin-v2-lr2e5-e5",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    "qwen3_30b_zhanlin_v2_lr1e5_e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8008/v1/chat/completions",
        "model": "/data/home/mingye/Qwen3-30B-A3B-sft-zhanlin-v2-lr1e5-e5",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    ...
    "qwen3_30b_augmented_lr5e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8000/v1/chat/completions",
        "model": "/data/home/mingye/models/Qwen3-30B-A3B-sft-augmented-combined-v0-lr5e5",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    "qwen3_30b_augmented_lr2e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8001/v1/chat/completions",
        "model": "/data/home/mingye/models/Qwen3-30B-A3B-sft-augmented-combined-v0-lr2e5",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    "qwen3_30b_augmented_rewritten_lr5e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8000/v1/chat/completions",
        "model": "/data/home/mingye/models/Qwen3-30B-A3B-sft-augmented-combined-v0-rewritten-lr5e5",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
    "qwen3_30b_augmented_rewritten_lr2e5": {
        "api_type": "openai",
        "api_url": "http://localhost:8001/v1/chat/completions",
        "model": "/data/home/mingye/models/Qwen3-30B-A3B-sft-augmented-combined-v0-rewritten-lr2e5",
        "api_key": "dummy",
        "extra_params": {},
        "context_window": 131072,
    },
...
# Docker image used as a lightweight sandbox for code_exec.  Any image with
# python3 + bash will do.  We pick the first available canonical-ms image.
_SANDBOX_IMAGE: str | None = None

def _get_sandbox_image() -> str | None:
    """Lazily find a usable Docker image for sandboxed code_exec."""
    global _SANDBOX_IMAGE
    if _SANDBOX_IMAGE is not None:
        return _SANDBOX_IMAGE if _SANDBOX_IMAGE != "" else None
    try:
        out = subprocess.run(
        ...
        for line in out.stdout.splitlines():
            if line.startswith("canonical-ms:"):
                _SANDBOX_IMAGE = line.strip()
                return _SANDBOX_IMAGE
    except Exception:
        pass
    _SANDBOX_IMAGE = ""  # mark as "not available"
    return None
...
def _local_code_exec(arguments, fs_root):
    ...
    When Docker is available, runs code inside a container with fs_root bind-
    mounted to /filesystem — giving proper sandbox isolation (find / won't leak
    host paths, /filesystem/... works like in the real eval Docker environment).
    Falls back to bare subprocess with path-rewrite hacks when Docker is absent.
    """
    ...
    if sandbox_image:
        # ── Docker sandbox path (clean) ──────────────────────────────────
        # Write code to a temp file, bind-mount fs_root to /filesystem,
        # and /tmp to /tmp so temp files are accessible.
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".sh", dir="/tmp", delete=False, prefix="agent_"
        ) as f:
            f.write(cmd)
            script_path = f.name
        os.chmod(script_path, 0o755)

        # Detect if it's Python code and wrap accordingly
        ...
        if inline_py:
            ...
            with open(script_path, "w") as f:
                f.write(py_code)
            docker_cmd = f"python3 /tmp/{os.path.basename(script_path)}"
        elif first_line.startswith(python_indicators):
            docker_cmd = f"python3 /tmp/{os.path.basename(script_path)}"
        else:
            docker_cmd = f"bash /tmp/{os.path.basename(script_path)}"

        try:
            result = subprocess.run(
                [
                    "docker", "run", "--rm",
                    "-v", f"{fs_root}:/filesystem",
                    "-v", "/tmp:/tmp",
                    "-w", "/filesystem",
                    "--network", "none",
                    sandbox_image,
        ...
        # ── Bare subprocess fallback (no Docker) ─────────────────────────
        # Rewrite /filesystem paths and mask output as best-effort workaround.
        if "/filesystem" in cmd:
            cmd = cmd.replace("/filesystem/", fs_root.rstrip("/") + "/")
            cmd = cmd.replace("/filesystem", fs_root.rstrip("/"))
        ...
        try:
            ...
            if result.returncode != 0:
                output += f"\n[Exit code: {result.returncode}]"
            # Mask real fs_root in output so model only sees /filesystem paths
            if fs_root and fs_root != "/filesystem":
                output = output.replace(fs_root.rstrip("/"), "/filesystem")
            return output if output else "(no output)"
        except subprocess.TimeoutExpired:
            return "Error: Command timed out after 300 seconds"
        except Exception as e:
...
def _get_tokenizer():
    """Lazily load the tokenizer for the active model."""
    global _tokenizer
    if _tokenizer is None:
        try:
            from transformers import AutoTokenizer
            _tokenizer = AutoTokenizer.from_pretrained(
                _cfg["model"], trust_remote_code=True
            )
        except Exception:
...
def _estimate_token_count(messages, tools=None):
    """Estimate token count for messages + tool defs.

    Uses the model's tokenizer when available (exact), falls back to
    chars // 3 (conservative) otherwise.
    """
    # Build a single string of all content for counting
    parts = []
    for msg in messages:
    ...
    if tokenizer is not None:
        try:
            ...
            pass
    # Fallback: chars // 3 (conservative, better than // 4 which underestimates)
...
def _truncate_messages_for_api(messages, tools=None):
    ...
    max_output_tokens = 8192  # must match _call_api_openai's max_tokens
...
def _call_api_openai(messages, tools, temperature=0.0):
    ...
    flat_messages = _flatten_tool_content(messages)

    # Dynamically cap max_tokens so input + output ≤ context_window
    context_window = _cfg.get("context_window", 131072)
    input_tokens = _estimate_token_count(messages)
    max_tokens = min(8192, max(256, context_window - input_tokens - 64))
