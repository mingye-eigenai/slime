...
# ── Cleanup ──────────────────────────────────────────────────────────────────
sudo docker rm -f slime-master 2>/dev/null || true
WORKER_IPS_CLEANUP=(92.38.143.26 85.234.91.98 85.234.91.76)
for _W in "${WORKER_IPS_CLEANUP[@]}"; do
  ssh -o StrictHostKeyChecking=no user@"${_W}" \
    "sudo docker rm -f slime-worker 2>/dev/null; true" &
done
wait
sleep 3
...
DOCKER_COMMON=(
  ...
  -e MODEL_ARGS_ROTARY_BASE=5000000
  -e PYTHONBUFFERED=16
)

# ── Master node training script (runs inside docker) ────────────────────────
cat > /data/train_235b_inner.sh << 'INNER_EOF'
#!/bin/bash
set -ex
ulimit -n 65536
...
INNER_EOF
chmod +x /data/train_235b_inner.sh

# ── Launch worker containers on remote nodes ─────────────────────────────────
for WORKER_IP in "${WORKER_IPS[@]}"; do
  echo "Starting Ray worker container on ${WORKER_IP}"
  ssh -o StrictHostKeyChecking=no user@"${WORKER_IP}" \
    "sudo docker run -d --name slime-worker \
      ${DOCKER_COMMON[@]} \
...
# ── Launch master container (foreground, runs training) ──────────────────────
