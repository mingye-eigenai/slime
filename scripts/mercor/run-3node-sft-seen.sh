...
APEX_TRAIN_DATA="${APEX_TRAIN_DATA:-/data/apex_docker_tasks_law_sft_seen.jsonl}"

# ── 3-Node Non-Colocate Config ───────────────────────────────────────────────
SAVE_DIR="${MODEL_DIR}/Qwen3-30B-A3B-sft-opus-mix-lr5e5-iter113_rl_3node_sft_seen"
...
ROLLOUT_ARGS=(
   ...
   --num-rollout 70
   --rollout-batch-size ${APEX_ROLLOUT_BATCH_SIZE:-8}
   --n-samples-per-prompt ${APEX_N_SAMPLES:-16}
   --rollout-max-response-len 131072
   --rollout-temperature 1.0

   --over-sampling-batch-size ${APEX_ROLLOUT_BATCH_SIZE:-8}
   --dynamic-sampling-filter-path slime.rollout.filter_hub.dynamic_sampling_filters.check_reward_nonzero_std

   --global-batch-size ${APEX_GLOBAL_BATCH_SIZE:-128}
   --balance-data
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-30B-3node-sft-seen
   --wandb-key ${WANDB_KEY}
)
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"APEX_ROLLOUT_DATA_DIR\": \"/data/apex_rollout_data/rl_3node_sft_seen\"
  }
}"
